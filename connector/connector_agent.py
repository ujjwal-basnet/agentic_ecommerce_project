"""Connector Agent — FastMCP server bridging SmartShop to external MCP clients.

This runs in an isolated subdirectory to avoid namespace collision with mcp.py
(the project's custom MCP protocol). It wraps SmartShop's internal agents via
FastMCP so external clients (ChatGPT, Claude Desktop, Facebook Messenger bots)
can interact with the shop using the standard MCP protocol.

Run:
    python connector/connector_agent.py                  # stdio (default)
    python connector/connector_agent.py --transport sse  # HTTP/SSE
"""
...
# ---------------------------------------------------------------------------
#  Bootstrap SmartShop backend (same pattern as __init__.py isolation)
# ---------------------------------------------------------------------------
import sys
...
# Remove parent from path when importing fastmcp/official-mcp
# so they don't accidentally import our mcp.py
_path_backup = [p for p in sys.path if str(parent) not in p]

# Import official MCP + FastMCP (they won't find our mcp.py)
from fastmcp import FastMCP as _FastMCP

# Restore path for SmartShop imports
sys.path = [str(parent)] + _path_backup

# Import SmartShop modules (they use our custom mcp.py)
import config
import database

database.init_db()

_log = logging.getLogger("smartshop.connector.agent")
...
mcp = _FastMCP(
    ...
    instructions=(
        ...
        "Or call individual tools: search_products, add_to_cart, view_cart, "
        "remove_from_cart, clear_cart, get_recommendations, get_weather, "
        "get_product_details, checkout. All responses are text-only."
    ),
)
...
def catalog_resource() -> str:
    ...
    catalog_path = parent / "product_description.md"
...
@mcp.resource("smartshop://session/{session_id}")
def session_resource(session_id: str) -> str:
    """Get cart summary and recent messages for a session."""
    ...
    messages = database.load_history(session_id, limit=10)

    lines = [f"Session: {session_id}", f"Cart: {count} items, Rs.{total}\n"]
    if messages:
...
def chat(message: str, session_id: str = "") -> str:
    ...
    if not plan:
        if config.openai_enabled():
            ...
            return reply
        return (
            "I'm SmartShop, your shopping assistant! "
            "Try asking about products, recommendations, or your cart."
        )
...
def add_to_cart(session_id: str, product_name: str, quantity: int = 1) -> str:
    ...
    database.db_add_to_cart(
        session_id, product["name"], float(product["price"]), quantity
    )
    cart = database.db_get_cart(session_id)
    count = sum(int(i.get("quantity", 0)) for i in cart)
    total = round(
    ...
    return (
        f"Added {quantity}x {product['name']} (Rs.{product['price']}) to cart. "
        f"Cart: {count} items, Rs.{total}"
    )
...
def view_cart(session_id: str) -> str:
    ...
    for item in cart:
        ...
        total += subtotal
        lines.append(
            f"  • {item['product_name']} — {qty}x Rs.{price} = Rs.{subtotal}"
        )
...
@mcp.tool
def update_cart(session_id: str, product_name: str, quantity: int) -> str:
...
def checkout(session_id: str) -> str:
    ...
    items_summary = ", ".join(
        f"{i['product_name']} x{i['quantity']}" for i in cart
    )
    return (
        f"Order placed!\n"
...
def get_recommendations() -> str:
    ...
    by_qty = sorted(
        in_stock, key=lambda x: int(x.get("quantity", 0)), reverse=True
    )
    recs = by_qty[:8]
...
# ── 8. Owner / Analytics (authenticated) ─────────────────────────────────

@mcp.tool
def get_analytics(days: int = 30) -> str:
    """Get shop analytics: revenue, orders, top products, stock levels.
    ...
    lines = [
        f"SmartShop Analytics (last {days} days)",
    ...
    for i, p in enumerate(top, 1):
        lines.append(
            f"  {i}. {p['product_name']} — "
            f"{p['total_sold']} sold, Rs.{p['revenue']} revenue"
        )

    lines.append("\nRevenue by Category:")
    for cat in by_cat:
        lines.append(f"  • {cat['category']}: Rs.{cat['revenue']}")

    low_stock = [s for s in stock if int(s.get("quantity", 0)) < 5]
    if low_stock:
        lines.append("\nLow Stock Items:")
        for s in low_stock:
            lines.append(f"  • {s['name']}: {s['quantity']} left")
...
def shopping_assistant() -> str:
    ...
    return (
        ...
        "- Use `add_to_cart` / `view_cart` / `remove_from_cart` / `update_cart` for cart\n"
        "- Use `get_recommendations` for product suggestions\n"
        "- Use `get_weather` for weather-based clothing advice\n"
        "- Use `get_analytics` for shop owner statistics\n"
...
def product_search_prompt(query: str) -> str:
    ...
    return (
        f"Search SmartShop for: {query}. "
        f"Use the search_products tool with this query."
    )
...
def _get_recent_context(session_id: str) -> str:
    ...
    try:
        messages = database.load_history(session_id, limit=6)
        if not messages:
            return ""
        lines = []
        for m in messages:
        ...
        if isinstance(products, list) and products:
            ...
            for p in products:
                if isinstance(p, dict):
                    ...
                    # Join first two with " — ", rest with spaces
                    if len(parts) > 1:
    ...
    if isinstance(data, list) and data:
        ...
        for p in data:
            if isinstance(p, dict):
                ...
                price = p.get("price", "")
                lines.append(
                    f"  • {name} — Rs.{price}" if price else f"  • {name}"
                )
        return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SmartShop MCP Connector Agent")
    parser.add_argument(
        "--transport",
        choices=["stdio", "sse"],
        default="stdio",
