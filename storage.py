...
# Buckets this app reads/writes. Created/upserted to public on startup.
...
def ensure_buckets() -> None:
    """Create or upsert the app's Supabase buckets to public visibility.

    A *private* bucket makes /object/public/... return "Bucket not found" and breaks
    Instagram image fetching. This runs on every startup so legacy private buckets
    are automatically fixed without manual Supabase dashboard intervention.

    Free tier: 1 GB storage + 5 GB egress/month — plenty for small deployments.
    """
    if not _configured():
    ...
    for name in BUCKETS:
        try:
            ...
            if r.status_code in (200, 201):
                ...
                continue
            # Already exists — force public visibility (fixes legacy private buckets).
            r2 = requests.put(
...
def upload(bucket: str, filename: str, data: bytes, content_type: str = "image/png") -> str | None:
    """Upload bytes to Supabase Storage bucket. Returns public URL, or None if unconfigured."""
    if not _configured():
        return None
