...
from api.integrations import email as email_receipts
...
async def chat_stream(
    ...
    async def event_generator():
        try:
            ...
            logger.exception(
                "chat_stream failed session=%s message=%r", session_id, message
            )
            yield {
                "event": "error",
                "data": json.dumps(
                    {"type": "error", "content": f"Something went wrong: {exc}"}
                ),
            }
            raise
...
async def checkout(session_id: str = Form(...)):
    ...
    if user and user.get("email"):
        await email_receipts.send_receipt(
            to_email=user["email"],
