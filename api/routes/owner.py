...
async def campaign_visual_generate(
    ...
    """
    from api.integrations.campaign import generate_campaign_image
    ...
    if not product_image_path or (
        ...
        raise HTTPException(
            status_code=400, detail=f"Product {product_id} has no usable image"
        )
    ...
    if (
        preset_key
        and preset_key in _BACKGROUND_PRESETS
        and background_image_path is None
    ):
...
async def campaign_launch(
    ...
    from api.routes.facebook import post_photo_to_page
    from api.routes.instagram import publish_photo_to_instagram
    ...
    if not image_url and image_bytes is not None:
        ...
        try:
            image_url = storage.upload(
                "campaign-images", filename, image_bytes, content_type=content_type
            )
        except Exception:
            _log.exception("Supabase upload for campaign launch failed")
        if not image_url and config.PUBLIC_BASE_URL:
            image_url = (
                f"{config.PUBLIC_BASE_URL.rstrip('/')}/data/campaigns/{filename}"
            )
    ...
    return {
        "ok": len(deployed) > 0 and not failed,
        "deployed": deployed,
        "failed": failed,
    }
