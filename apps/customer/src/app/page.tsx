    ...
    await fetchSSE(
      ...
      (event) => {
        if (event.type === "text") {
          ...
          setMessages((prev) => [...prev, { id: genId(), role: "assistant", text: event.content || "Request failed.", ts: timeNow() }]);
          scrollToBottom();
        }
      },
      () => setLoading(false),
        ...
        <div className="flex justify-between items-center w-full px-6 h-16 max-w-screen-2xl mx-auto">
          ...
          <nav className="hidden md:flex items-center space-x-8 font-headline font-medium tracking-tight text-sm">
            <button
              ...
              className={`pb-1 transition-colors ${view === "chat" ? "text-on-surface border-b border-on-surface" : "text-outline hover:text-on-surface"}`}
            >
              Chat
            </button>
            <button
              onClick={() => setView("orders")}
              className={`pb-1 transition-colors ${view === "orders" ? "text-on-surface border-b border-on-surface" : "text-outline hover:text-on-surface"}`}
            >
              Orders
            </button>
          </nav>
            ...
            {messages.length === 0 && (
              <div className="text-center mb-16 mt-8">
                ...
                <div className="flex flex-wrap gap-2 mt-8 justify-center">
                  {["Show me red shirts", "Recommend something", "Show me laptops", "View my cart"].map((q) => (
                    <button
                      key={q}
